...
{
	// 用curl命令抓.bin文件
	char curl_cmd[] = "curl -O http://webbtc.com/block/";
	int new_size = strlen(argv[1]) + strlen(curl_cmd) + 1;
	char *new_buffer = (char *)malloc(new_size);
	strcpy(new_buffer, curl_cmd);
	strcat(new_buffer, argv[1]);
	system(new_buffer);
	free(new_buffer);
	new_buffer = NULL;
